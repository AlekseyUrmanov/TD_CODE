...
class db_operator:
    ...
    def create_table_div(self, table_name):
    ...
    def create_table_spl(self, table_name, strikes):

        create_table_sql = "CREATE TABLE IF NOT EXISTS {} (id INTEGER PRIMARY KEY, {});".format(
            table_name, ", ".join("[{}] TEXT".format(col) for col in strikes)
        )
    ...
    def add_strdPL_data(self, row, table_name):
        ...
        insert_data_sql = "INSERT INTO {} ({}) VALUES ({});".format(
            table_name, ", ".join('"' + key + '"' for key in row.keys()),
